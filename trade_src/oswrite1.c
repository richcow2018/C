...
/* ＜ソース修正者＞ 			 	 サプライ　朱												*/
/* ＜ソース正年月日及び修正ＩＤ＞	 ２００３年　７月１１日     故障管理番号（ＳＳＬＡ００４３）*/
/* ＜ソース修正者＞ 			 	 サプライ　朱												*/
/* ＜ソース正年月日及び修正ＩＤ＞	 ２００３年　８月　８日     故障管理番号（ＴＢＣＳ０２６７）*/
...
/*****	故障管理番号TBCS0267の対応		Start							修正日：2003.08.08	*****/
	char cErrMsgFlg = 0;									// エラーメッセージフラグ
															// 0：通常メッセージ出力
															// 1：KO00メッセージ出力
/*****	故障管理番号TBCS0267の対応		 End							修正日：2003.08.08	*****/
...
//	DbgMsg01( DLv04, ( OutPut, "*** os_SendCon (1)\n" ) );						
	...
	if(lRet < 0 )
	{
															// KO00 メッセージ識別子の検索
															// SSLA0043
		lRet = os_SearchMsgID(ptEmmcInd, ptEmmc->lConCnt, NOT_FOUND);
/*****	故障管理番号TBCS0267の対応		Start							修正日：2003.08.08	*****/
		cErrMsgFlg = 1;										// エラーメッセージフラグを設定
/*****	故障管理番号TBCS0267の対応		 End							修正日：2003.08.08	*****/
	}
...
//								 pcTmpBufMsgId[3]);
															...
															// not foundメッセージを編集 SSLA0043
		lRetForSPrintf = sprintf(cNotFoundMsg, "[ERR    ] %.4s* CMES %.4s not found", 
								 NOT_FOUND, pcTmpBufMsgId);
...
//		os_trcget2( , D_T_SYS_MALLOC, 1, lBufLen + lMsgLen + 20 );		
															
															// TBCS0123	
//		pcTmpForMsg = (char*)malloc( (lBufLen * 2) + lMsgLen);
		
															// TBCS0153/TBCS0155
		os_trcget2( D_MD_OS_SENDCON, D_T_SYS_MALLOC, 1, ( (lBufLen * 2) + (lMsgLen + 9) ) );
															// TBCS0153/TBCS0155
															// 全角に編集するサイズ分=引数*2
															// 時間,スペース,[]のサイズ分=9
		...
		os_trcget2( D_MD_OS_SENDCON, D_T_SYS_END, 1, pcTmpForMsg );	
															
		/******************************************/
		/* メッセージを編集する					  */
		/******************************************/
/*****	故障管理番号TBCS0267の対応		Start							修正日：2003.08.08	*****/
		if (1 == cErrMsgFlg)								// エラーメッセージフラグが「1」の場合
		{													// （KO00メッセージ出力）
			cNotFoundMsg[0]='\t';							// 区切文字を設定
															// メッセージ識別子を設定
			strncpy(&cNotFoundMsg[1], pcTmpBufMsgId, sizeof(pcTmpBufMsgId));

															// メッセージを編集
			os_GetMsg(pcFormatMsg, cNotFoundMsg, &pcTmpMsg,
								sizeof(pcTmpBufMsgId)+1, sConAfter, lMsgLen);
		}
		else												// 通常メッセージ出力
		{
															// メッセージを編集
...
//								pcPriority, lhr, lmm, pcTmpMsg);
															...
															// SSLA0043
		lRetForSPrintf = sprintf( pcTmpForMsg, "[%s] %s", pcPriority, pcTmpMsg);
	
		DbgMsg01( DLv04, ( OutPut, "*** os_SendCon (7) pcFormatMsg == %s\n", pcFormatMsg) );
		
		if(lRetForSPrintf < 0)								// 出力メッセージの編集
		{													// ができなかった場合
			os_trcget2( D_MD_OS_SENDCON, D_T_SYS_FREE, 1, pcTmpMsg );

			free(pcTmpMsg);									// 領域を解放
			
			os_trcget2( D_MD_OS_SENDCON, D_T_SYS_END, 0 );
			
			os_trcget2( D_MD_OS_SENDCON, D_T_SYS_FREE, 1, pcTmpForMsg );
...
//	DbgMsg01( DLv04, ( OutPut, "*** os_SendLog (1)\n" ) );	
	...
	if(lRet < 0 )
	{
															// KO00 メッセージ識別子の検索
															// SSLA0043
		lRet = os_SearchMsgID(ptEmmcInd, ptEmmc->lLogCnt, NOT_FOUND);
/*****	故障管理番号TBCS0267の対応		Start							修正日：2003.08.08	*****/
		cErrMsgFlg = 1;										// エラーメッセージフラグを設定
/*****	故障管理番号TBCS0267の対応		 End							修正日：2003.08.08	*****/
	}
...
//								 pcTmpBufMsgId[2], pcTmpBufMsgId[3]);
															...
															// not foundメッセージを編集 SSLA0043
		lRetForSPrintf = sprintf(cNotFoundMsg, "[ERR    ] %.4s* LMES %.4s not found", 
								 NOT_FOUND, pcTmpBufMsgId);
...
//		pcTmpForMsg = (char*)malloc( (lBufLen * 2) + lMsgLen);
																	...
																	// TBCS0153/TBCS0155
		os_trcget2( D_MD_OS_SENDLOG, D_T_SYS_MALLOC, 1, ( (lBufLen * 2) + (lMsgLen + 9) ) );
															// TBCS0153/TBCS0155
															// 全角に編集するサイズ分=引数*2
															// 時間,スペース,[]のサイズ分=9
		pcTmpForMsg = (char*)malloc( (lBufLen * 2) + (lMsgLen + 9) );
		
		os_trcget2( D_MD_OS_SENDLOG, D_T_SYS_END, 1, pcTmpForMsg );	
...
//								pcPriority, lhr, lmm, pcTmpMsg);
		...
		{													// できなかった場合
			os_trcget2( D_MD_OS_SENDLOG, D_T_SYS_FREE, 1, pcTmpMsg );
	
			free(pcTmpMsg);									// 領域を解放
			
			os_trcget2( D_MD_OS_SENDLOG, D_T_SYS_END, 0 );
				
			os_trcget2( D_MD_OS_SENDLOG, D_T_SYS_FREE, 1, pcTmpForMsg );	
...
/*****	故障管理番号TBCS0267の対応		 End							修正日：2003.08.08	*****/
	...
	if(lRet < 0 )
	{
															// KO00 メッセージ識別子の検索
															// SSLA0043
		lRet = os_SearchMsgID(ptEmmcInd, ptEmmc->lProcCnt, NOT_FOUND);
/*****	故障管理番号TBCS0267の対応		Start							修正日：2003.08.08	*****/
		cErrMsgFlg = 1;										// エラーメッセージフラグを設定
/*****	故障管理番号TBCS0267の対応		 End							修正日：2003.08.08	*****/
	}
...
//	DbgMsg01( DLv04, ( OutPut, "\n*** os_SendProc (1.2) after os_SearchMsgID\n") );
	...
	{														// 見つからなかった場合
		...
		if(0 == ptEmmc->lPCode)
		{
			...
			os_trcget2( D_MD_OS_SENDPROC, D_T_ERR_PRA, 1, PRC_ERROR );
...
//								 pcTmpBufMsgId[2], pcTmpBufMsgId[3]);
															...
															// not foundメッセージを編集
		lRetForSPrintf = sprintf(cNotFoundMsg+1, "%.2d:%.2d%.4s* SMES %.4s not found", 
								 lhr, lmm, NOT_FOUND, pcTmpBufMsgId);
...
//		pcTmpForMsg = (char*)malloc( (lBufLen * 2) + lMsgLen);
															...
															// TBCS0153/TBCS0155
		os_trcget2( D_MD_OS_SENDPROC, D_T_SYS_MALLOC, 1, ( (lBufLen * 2) + (lMsgLen + 9) ) );
															// TBCS0153/TBCS0155
															// 全角に編集するサイズ分=引数*2
															// 時間,スペース,[]のサイズ分=9
		pcTmpForMsg = (char*)malloc( (lBufLen * 2) + (lMsgLen + 9) );
		
		os_trcget2( D_MD_OS_SENDPROC, D_T_SYS_END, 1, pcTmpForMsg );		
...
/*****	故障管理番号TBCS0267の対応		 End							修正日：2003.08.08	*****/
		...
		if(lRetForSPrintf < 0)								// 出力メッセージの編集
		{													// ができなかった場合
			os_trcget2( D_MD_OS_SENDPROC, D_T_SYS_FREE, 1, pcTmpMsg );
			
			free(pcTmpMsg);									// 領域を解放
	
			os_trcget2( D_MD_OS_SENDPROC, D_T_SYS_END, 0 );
			
			os_trcget2( D_MD_OS_SENDPROC, D_T_SYS_FREE, 1, pcTmpForMsg );
...
//			DbgMsg01( DLv04, ( OutPut, "*** os_SendProc (0) \n" ) );	
		...
		else
		{
			...
			os_trcget2( D_MD_OS_SENDPROC, D_T_TRA_OUT, 1, PRC_NORMAL );
