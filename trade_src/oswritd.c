...
/***************************** 利用者作成ヘッダ・ファイル *******************/
#include "oswritd.h"
#include "oswritd_ins.h"                // 内部参照用（個別）
#include "osseek.h"                     // osseek用
#include "oswrite.h"                    // oswrite用
...
/****************************************************************************/
    ...
    if( len < D_RECORD_NUM )            // 書込みレコード数のチェック
    {
...
//    										D_PROC_NAME, ptTEflcTableInd->pFilStm ) );
	...
	lCurPos = ftell( ptTEflcTableInd->pFilStm );         // カレントポジションを取得する
