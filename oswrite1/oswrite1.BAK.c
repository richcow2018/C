...
//		pcTmpForMsg = (char*)malloc( (lBufLen * 2) + lMsgLen);
		...
		memset(pcTmpForMsg, 0x00, ( (lBufLen * 2) + (lMsgLen + 9) ) );
		...
		DbgDmp02( DLv04, ("## AFTER SENDLOG ",  pcTmpForMsg, lRetForSPrintf+1,1,0));
		
		DbgMsg01( DLv04, ( OutPut, "*** os_SendCon (7)pcTmpForMsg == %s\n", pcTmpForMsg) );
	
		DbgMsg01( DLv04, ( OutPut, "*** os_SendCon (7.1) pcFormatMsg == %s\n", pcFormatMsg) );
...
//	*pcMsg = (char*)malloc( (lBufLen * 2) + lMsgLen);		// の領域を取ります
	...
	*pcMsg = (char*)malloc( (lBufLen * 2) + (lMsgLen + 9)); // データ編集の領域を確保
	...
	memset(*pcMsg, 0x00, ( (lBufLen * 2) + (lMsgLen + 9) ) );
	...
	if ( lMsgStart != -1) 									// 見つかった場合
	{
		...
		DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg(4.1) lMsgStart\n" ) );

//		os_EditString((pcFmtAdr + 8), *pcMsg, 8, lMsgStart);
		
		DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg(4.2) pcFmtAdr =%s\n", pcFmtAdr + 8 ) );
		
		DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg(4.3) *pcMsg =%s\n", *pcMsg ) );
...
//					lMsgStart++;
		...
		//		lMsgStart = lMsgEnd;
			...
			else
			{
				...
				DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (13.5) lMsgLen  == %ld\n", 
				lMsgLen ) );				

				DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (13.6)  lMsgStart + 2 == %ld\n", 
				lMsgStart + 2) );				

#ifdef debug
				if ((lMsgStart + 2) < lMsgLen)
				{ 				
					DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (13.5) pcFmtAdr  == %s\n", 
					pcFmtAdr[lMsgStart + 2] ) );
				
					DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (13.6) pcFmtAdr  == %c\n", 
					pcFmtAdr[lMsgStart + 2] ) );
				}
#endif

				DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (13.8) lMsgStart  == %ld\n", 
				lMsgStart ) );
			}
			

				DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (13.9) ltemp  == %ld\n", 
				ltemp ) );

#ifndef debug_test			

//		DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (13.7) lMsgLen  == %ld\n", 
//				lMsgLen ) );	
	...
	//	DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (8) pcFmtAdr == %s\n", pcFmtAdr) );	
		...
		DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg(16.1) lMsgStart\n" ) );
		
		strncpy(*pcMsg, (pcFmtAdr + 8), lMsgLen); 			// 指定範囲されたメッセージ情報か
															// 出力メッセージに加え
	} 

	DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (17) pcFmtAdr  == %s\n", 
					pcFmtAdr ) );
					
					
	DbgMsg01( DLv04, ( OutPut, "*** os_GetMsg (18) length  == %ld\n", 
					strlen(pcFmtAdr) ) );
...
long os_MsgOut(char *pcMsgFormat, char *pcKey, char *pcPriority)
{
	...
	memset(ltemp, 0x00, 512);
	
	memcpy(ltemp, pcMsgFormat, strlen(pcMsgFormat)); 

	syslog(lRetSys, ltemp);						// syslog システムコール 

	
//	syslog(lRetSys, pcMsgFormat);						// syslog システムコール 
	
	DbgDmp02( DLv04, ("## AFTER SENDSYSLOG ",  pcMsgFormat, strlen(pcMsgFormat) + 1,1,0));
	
	DbgDmp02( DLv04, ("## AFTER SENDSYSLOG1 ",  pcMsgFormat, 512,1,0));
